...
_canvas_resize_cb(Ecore_Evas *ee)
{
   ...
   efl_gfx_object_size_set(d.bg, EINA_SIZE2D(w,  h));
}
   ...
   if (strcmp(ev->key, "v") == 0) /* toggle clipper's visibility */
     {
        ...
        visibility = efl_gfx_object_visible_get(d.clipper);
        efl_gfx_object_visible_set(d.clipper, !visibility);
...
main(void)
{
   ...
   /* white bg */
   efl_gfx_object_position_set(d.bg, EINA_POSITION2D(0, 0));
   /* at canvas' origin */
   efl_gfx_object_size_set(d.bg, EINA_SIZE2D(WIDTH,  HEIGHT));
   /* covers full canvas */
   efl_gfx_object_visible_set(d.bg, EINA_TRUE);
   ...
   if (err != EVAS_LOAD_ERROR_NONE)
     ...
     {
        efl_gfx_object_position_set(d.img, EINA_POSITION2D(0, 0));
        efl_gfx_object_size_set(d.img, EINA_SIZE2D(WIDTH,  HEIGHT));
        efl_gfx_object_visible_set(d.img, EINA_TRUE);
   ...
   if (err != EVAS_LOAD_ERROR_NONE)
   ...
   else
     {
        ...
        efl_gfx_object_position_set(d.clipper_border, EINA_POSITION2D((WIDTH / 4) -3, (HEIGHT / 4) - 3));
        efl_gfx_object_size_set(d.clipper_border, EINA_SIZE2D((WIDTH / 2) + 6,  (HEIGHT / 2) + 6));
        efl_gfx_object_visible_set(d.clipper_border, EINA_TRUE);
     }
   ...
   efl_gfx_object_position_set(d.clipper, EINA_POSITION2D(WIDTH / 4, HEIGHT / 4));
   efl_gfx_object_size_set(d.clipper, EINA_SIZE2D(WIDTH / 2,  HEIGHT / 2));
   efl_gfx_object_visible_set(d.clipper, EINA_TRUE);
