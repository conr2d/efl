...
{
   Eo *win = (Eo*) data;
   efl_gfx_object_visible_set(win, EINA_FALSE);
   return EINA_FALSE;
}
   ...
   if (elm_win_xwindow_get(win))
     {
        ...
        efl_gfx_object_visible_set(win, EINA_TRUE);
        ...
        Eina_Bool visible;
        visible = efl_gfx_object_visible_get(win);
...
EFL_START_TEST (efl_ui_win_multi_touch_inputs)
{
   ...
   elm_win_autohide_set(win, EINA_TRUE);
   efl_gfx_object_visible_set(win, EINA_TRUE);
   efl_gfx_object_size_set(win, EINA_SIZE2D(100,  100));
