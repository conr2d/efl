...
{
   efl_gfx_object_size_set(o_bg, EINA_SIZE2D(w,  h));
}
...
bg_setup(void)
{
   ...
   efl_gfx_object_position_set(o_bg, EINA_POSITION2D(0, 0));
   efl_gfx_object_size_set(o_bg, EINA_SIZE2D(startw,  starth));
   efl_gfx_stack_layer_set(o_bg, -999);
   efl_gfx_object_visible_set(o_bg, EINA_TRUE);
...
video_obj_time_changed(Evas_Object *obj, Evas_Object *edje)
{
   ...
   efl_text_container_text_set(efl_part(edje, "video_progress_txt"), buf);
}
...
video_obj_frame_resize_cb(void *data, const Efl_Event *event)
{
   ...
   efl_gfx_object_size_set(oe, EINA_SIZE2D(w,  h));
...
video_obj_signal_alpha_cb(void *data, Evas_Object *o, const char *emission EINA_UNUSED, const char *source)
{
   ...
   efl_text_container_text_set(efl_part(o, "video_alpha_txt"), buf);
}
...
video_obj_signal_vol_cb(void *data, Evas_Object *o, const char *emission EINA_UNUSED, const char *source)
{
   ...
   efl_text_container_text_set(efl_part(o, "video_volume_txt"), buf);
}
   ...
   if (fd->moving)
     {
        ...
        evas_pointer_canvas_xy_get(evas_object_evas_get(o), &x, &y);
        pos = efl_gfx_object_position_get(o);
        pos.x += (x - fd->x);
        pos.y += (y - fd->y);
        efl_gfx_object_position_set(o, pos);
        fd->x = x;
        fd->y = y;
     }
   else if (fd->resizing)
...
init_video_object(const char *module_filename, const char *filename)
{
   ...
   efl_gfx_object_position_set(oe, EINA_POSITION2D(offset, offset));
   edje_object_size_min_calc(oe, &w, &h);
   efl_gfx_object_size_set(oe, EINA_SIZE2D(w,  h));
   ...
   efl_text_container_text_set(efl_part(oe, "video_alpha_txt"), "alpha 255");
   efl_ui_drag_value_set(efl_part(oe, "video_volume"), 0.0, 0.5);
   efl_text_container_text_set(efl_part(oe, "video_volume_txt"), "vol 0.50");
   efl_layout_signal_emit(oe, "video_state", "play");
   efl_gfx_object_visible_set(oe, EINA_TRUE);
}
