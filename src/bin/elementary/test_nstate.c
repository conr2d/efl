...
_state_changed_cb(void *data EINA_UNUSED, const Efl_Event *ev)
{
   ...
   efl_text_container_text_set(ev->object, eina_slstr_printf("nstate = %d", val));
}
...
test_nstate(void *data EINA_UNUSED, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   ...
   win = efl_add(EFL_UI_WIN_CLASS, efl_main_loop_get(),
                 efl_text_container_text_set(efl_added, "N-state widget"),
   ...
   efl_text_container_text_set(nstate, "nstate = 0");
   efl_pack(bx, nstate);

   efl_gfx_object_size_set(win, EINA_SIZE2D(120, 80));
}
