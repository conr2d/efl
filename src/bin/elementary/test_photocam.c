...
_zoomable_move_resize_cb(void *data, const Efl_Event *ev)
{
   ...
   r = efl_gfx_object_geometry_get(ev->object);
   efl_gfx_object_size_set(data, EINA_SIZE2D(r.w,  r.h));
   efl_gfx_object_position_set(data, r.pos);
}
...
test_image_zoomable_animated(void *data EINA_UNUSED, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   ...
   win = efl_add(EFL_UI_WIN_CLASS, efl_main_loop_get(),
                 ...
                 efl_text_container_text_set(efl_added, "Efl.Ui.Image_Zoomable animation"),
   ...
   efl_add(EFL_UI_TEXT_CLASS, bx,
           efl_text_container_text_set(efl_added, "Clicking the image will play/pause animation."),
           efl_text_container_interactive_editable_set(efl_added, EINA_FALSE),
   ...
   // add move/resize callbacks to resize rect manually
   efl_event_callback_add(zoomable, EFL_GFX_OBJECT_EVENT_RESIZE, _zoomable_move_resize_cb, rect);
   efl_event_callback_add(zoomable, EFL_GFX_OBJECT_EVENT_MOVE, _zoomable_move_resize_cb, rect);

   efl_gfx_object_size_set(win, EINA_SIZE2D(300,  320));
}
