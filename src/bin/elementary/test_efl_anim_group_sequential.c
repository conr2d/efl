   ...
   if (ad->is_btn_visible)
     {
        ...
        efl_text_container_text_set(obj, "Start Sequential Group Animation to hide button");
     }
   else
     {
        //Create Animation Object from Animation
        efl_animation_player_animation_set(ad->anim_obj, ad->sequential_hide_anim);
        efl_text_container_text_set(obj, "Start Sequential Group Animation to show button");
     }
