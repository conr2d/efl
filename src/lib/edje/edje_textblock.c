   ...
   if ((chosen_desc->text.min_x) || (chosen_desc->text.min_y))
     {
        ...
        if (!chosen_desc->text.min_x)
          {
             efl_gfx_object_size_set(ep->object, EINA_SIZE2D(TO_INT(params->eval.w),  TO_INT(params->eval.h)));
        ...
        if (!chosen_desc->text.min_x)
          {
             ...
             if (temp_w > 0)
               {
                  efl_gfx_object_size_set(ep->object, EINA_SIZE2D(temp_w,  temp_h));
                  efl_canvas_text_size_formatted_get(ep->object, &tw, &th);
             ...
             else
               {
                  ...
                  if (chosen_desc->text.max_y)
                  ...
                  else
                    {
                       ...
                       temp_h = efl_gfx_object_size_get(ep->object).h;
                       efl_gfx_object_size_set(ep->object, EINA_SIZE2D(temp_w,  temp_h));
                       efl_canvas_text_size_formatted_get(ep->object, &tw, &th);
        ...
        if ((chosen_desc->text.fit_x) || (chosen_desc->text.fit_y))
          {
             ...
             efl_gfx_object_scale_set(ep->object, base_s);
             efl_canvas_text_size_native_get(ep->object, &tw, &th);
              ...
              * and less influenced by hinting... */
             {
                ...
                efl_gfx_object_scale_set(ep->object, orig_s);
                efl_canvas_text_size_native_get(ep->object, &tw, &th);
             }
                  ...
                  if (tw > 0)
                    {
                       s = _edje_part_recalc_single_textblock_scale_range_adjust(chosen_desc, base_s,
                                                                                 orig_s * TO_INT(params->eval.w) / tw);
                       efl_gfx_object_scale_set(ep->object, s);
