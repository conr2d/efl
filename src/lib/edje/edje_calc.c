 ...
 */
{
   ...
   sz = efl_gfx_object_size_get(ep->object);
   mw = sz.w;
   mh = sz.h;
   ...
   if (chosen_desc)
     {
        ...
        if ((chosen_desc->text.min_x) || (chosen_desc->text.min_y) ||
            (chosen_desc->text.max_x) || (chosen_desc->text.max_y))
          {
             ...
             ts = efl_gfx_object_size_get(ep->object);
             if (chosen_desc->text.max_x)
               {
                  int l, r;
           ...
           case EDJE_PART_TYPE_EXTERNAL:
...
#ifdef HAVE_EPHYSICS
             efl_gfx_object_size_set(ep->object, EINA_SIZE2D(pf->final.w,  pf->final.h));
             if ((ep->part->physics_body) && (!ep->body))
               {
               ...
               efl_gfx_object_position_set(ep->object, EINA_POSITION2D(ed->x + pf->final.x, ed->y + pf->final.y));
#else
             efl_gfx_object_position_set(ep->object, EINA_POSITION2D(ed->x + pf->final.x, ed->y + pf->final.y));
             efl_gfx_object_size_set(ep->object, EINA_SIZE2D(pf->final.w,  pf->final.h));
#endif
             ...
             if (ep->nested_smart) /* Move, Resize all nested parts */
               {   /* Not really needed but will improve the bounding box evaluation done by Evas */
                  efl_gfx_object_position_set(ep->nested_smart, EINA_POSITION2D(ed->x + pf->final.x, ed->y + pf->final.y));
                  efl_gfx_object_size_set(ep->nested_smart, EINA_SIZE2D(pf->final.w,  pf->final.h));
               }
           ...
           case EDJE_PART_TYPE_CAMERA:
             {
                ...
                efl_gfx_object_size_set(ep->object, EINA_SIZE2D(pf->req.w,  pf->req.h));
                ...
                efl_gfx_object_size_set(viewport, EINA_SIZE2D(pf->req.w,  pf->req.h));
             ...
             if (pf->visible)
               {
                  ...
                  efl_gfx_object_position_set(ep->typedata.swallow->swallowed_object, EINA_POSITION2D(ed->x + pf->final.x, ed->y + pf->final.y));
                  efl_gfx_object_size_set(ep->typedata.swallow->swallowed_object, EINA_SIZE2D(pf->final.w,  pf->final.h));
                  efl_gfx_object_visible_set(ep->typedata.swallow->swallowed_object, vis);
               }
