...
_efl_canvas_layout_part_swallow_efl_content_content_set(Eo *obj, void *_pd EINA_UNUSED, Efl_Gfx_Object *content)
{
   PROXY_DATA_GET(obj, pd);
...
{
   PROXY_DATA_GET(obj, pd);
   Efl_Gfx_Object *content = _edje_efl_content_content_get(pd->ed, pd->part);
