...
static void
_eio_model_info_build(Eio_Model *model, Eio_Model_Data *pd)
{
   char *path;
...
static void
_eio_build_st(Eio_Model *model, Eio_Model_Data *pd)
{
   if (pd->st) return ;
...
_eio_build_mime_now(void *data, const Eina_Value v, const Eina_Future *dead_future EINA_UNUSED)
{
   ...
   if (ecore_time_get() - ecore_loop_time_get() > 0.004)
     {
        return eina_future_as_value(eina_future_then(efl_loop_job(pd->loop),
...
static void
_eio_build_mime(Efl_Object *model, Eio_Model_Data *pd)
{
   Eina_Future *f;
...
static Eina_Value *
_property_filename_cb(Eo *obj, Eio_Model_Data *pd)
{
   _eio_model_info_build(obj, pd);
   if (pd->info)
     return eina_value_string_new(pd->info->path + pd->info->name_length);
   return eina_value_error_new(EAGAIN);
}

static Eina_Value *
_property_path_cb(Eo *obj EINA_UNUSED, Eio_Model_Data *pd)
{
   return eina_value_string_new(pd->path);
}

static Eina_Value *
_property_direct_info_cb(Eo *obj, Eio_Model_Data *pd)
{
   _eio_model_info_build(obj, pd);
...
#define TIMECB(Prop)                                                    \
  static Eina_Value *                                                   \
  _property_##Prop##_cb(Eo *obj, Eio_Model_Data *pd)                    \
  {                                                                     \
     if (pd->st)                                                        \
...
static Eina_Value *
_property_is_dir_cb(Eo *obj, Eio_Model_Data *pd)
{
   if (pd->st)
...
static Eina_Value *
_property_is_lnk_cb(Eo *obj, Eio_Model_Data *pd)
{
   if (pd->st)
...
static Eina_Value *
_property_size_cb(Eo *obj, Eio_Model_Data *pd)
{
   if (pd->st)
...
static Eina_Value *
_property_stat_cb(Eo *obj, Eio_Model_Data *pd)
{
   if (pd->st)
     {
        Eina_Value *r;
...
static Eina_Value *
_property_mime_type_cb(Eo *obj, Eio_Model_Data *pd)
{
   if (pd->mime_type)
     return eina_value_string_new(pd->mime_type);

   _eio_build_mime(obj, pd);
   return eina_value_string_new("application/octet-stream");
}
...
static struct {
   const char *name;
   Eina_Value *(*cb)(Eo *obj, Eio_Model_Data *pd);
} properties[] = {
  PP(filename), PP(path),
  PP(direct_info),
 ...
 */
static Eina_Array *
_eio_model_efl_model_properties_get(Eo *obj EINA_UNUSED,
...
static Eina_Value *
_eio_model_efl_model_property_get(Eo *obj, Eio_Model_Data *pd, const char *property)
{
   unsigned int i;
 ...
 */
static unsigned int
_eio_model_efl_model_children_count_get(Eo *obj, Eio_Model_Data *pd)
{
   ...
   if (!pd->info)
            ...
            pd->info->type == EINA_FILE_DIR)
     {
        ...
        f = efl_io_manager_direct_ls(iom, pd->path, EINA_FALSE,
                                     obj, _eio_model_children_list, NULL);
...
static const char *
_eio_model_path_get(Eo *obj EINA_UNUSED, Eio_Model_Data *priv)
{
   return priv->path;
}
