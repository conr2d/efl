        ...
        if (!efl_isa(elem, EFL_UI_WIDGET_CLASS))
          {
             if (efl_isa(elem, EFL_UI_FOCUS_OBJECT_MIXIN))
             ...
             else if (efl_isa(elem, EFL_GFX_OBJECT_INTERFACE))
               {
               ...
               {
                  EINA_SAFETY_ERROR("List contains element that is not EFL_UI_FOCUS_OBJECT_MIXIN or EFL_GFX_OBJECT_INTERFACE or EFL_UI_WIDGET_CLASS");
                  continue;
               }
          }
...
EFL_CALLBACKS_ARRAY_DEFINE(canvas_obj,
    {EFL_GFX_OBJECT_EVENT_RESIZE, _new_geom},
    {EFL_GFX_OBJECT_EVENT_MOVE, _new_geom},
...
{
   return efl_gfx_object_geometry_get(pd->object);
}
