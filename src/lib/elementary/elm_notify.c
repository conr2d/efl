...
_elm_notify_efl_ui_widget_theme_apply(Eo *obj, Elm_Notify_Data *sd)
{
   ...
   edje_object_scale_set
     (sd->notify, efl_gfx_object_scale_get(obj) * elm_config_scale_get());
...
EOLIAN static void
_elm_notify_efl_gfx_object_size_set(Eo *obj, Elm_Notify_Data *sd, Eina_Size2D sz)
{
   ...
   if (!sd->parent && sd->content)
     {
        ...
        pos = efl_gfx_object_position_get(obj);
        _notify_move_to_orientation(obj, pos.x, pos.y, sz.w, sz.h);
     }
}

EOLIAN static void
_elm_notify_efl_gfx_object_position_set(Eo *obj, Elm_Notify_Data *sd, Eina_Position2D pos)
{
...
EOLIAN static void
_elm_notify_efl_gfx_object_visible_set(Eo *obj, Elm_Notify_Data *sd, Eina_Bool vis)
{
                  ...
                  const char *source EINA_UNUSED)
{
   ...
   evas_object_hide(sd->notify);
   if (!sd->allow_events) evas_object_hide(sd->block_events);
   efl_gfx_object_visible_set(efl_super(data, MY_CLASS), EINA_FALSE);
