   ...
   if (!sd->sel_handler_disabled)
     {
        ...
        efl_text_container_interactive_selection_cursors_get(obj, &sel_start, &sel_end);
                   ...
                   Elm_Selection_Data *sel_data)
{
   ...
   if ((sel_data->format & ELM_SEL_FORMAT_IMAGE) &&
   ...
   else if (sd->cnp_mode == ELM_CNP_MODE_PLAINTEXT)
     {
        ...
        efl_text_container_interactive_selection_cursors_get(obj, &start, &end);
   ...
   if (sd->scroll)
     {
        ...
        if (vw > minw) minw = vw;
        efl_gfx_object_size_set(sd->entry_edje, EINA_SIZE2D(minw,  minh));
   ...
   if (sd->scroll)
     {
        Evas_Coord vw = 0, vh = 0;
        elm_interface_scrollable_content_viewport_geometry_get(obj, NULL, NULL, &vw, &vh);
        efl_gfx_object_size_set(sd->entry_edje, EINA_SIZE2D(vw, vh));
     }
   if (can_async)
     {
        Eina_Future *f;
                             ...
                             void *event_info)
{
   ...
   /* Get the cursors */
   efl_text_container_interactive_selection_cursors_get(text_obj, &sel_start, &sel_end);
...
EOLIAN static void
_efl_ui_text_efl_gfx_object_position_set(Eo *obj, Efl_Ui_Text_Data *sd, Eina_Position2D pos)
{
   if (_evas_object_intercept_call(obj, EVAS_OBJECT_INTERCEPT_CB_MOVE, 0, pos.x, pos.y))
     return;

   efl_gfx_object_position_set(efl_super(obj, MY_CLASS), pos);
   efl_gfx_object_position_set(sd->hit_rect, pos);
...
EOLIAN static void
_efl_ui_text_efl_gfx_object_size_set(Eo *obj, Efl_Ui_Text_Data *sd, Eina_Size2D sz)
{
   if (_evas_object_intercept_call(obj, EVAS_OBJECT_INTERCEPT_CB_RESIZE, 0, sz.w, sz.h))
     return;

   efl_gfx_object_size_set(sd->hit_rect, sz);
   _update_selection_handler(obj);
...
EOLIAN static void
_efl_ui_text_efl_gfx_object_visible_set(Eo *obj, Efl_Ui_Text_Data *sd EINA_UNUSED, Eina_Bool vis)
{
...
_efl_ui_text_efl_object_constructor(Eo *obj, Efl_Ui_Text_Data *sd)
{
   ...
   efl_text_container_interactive_editable_set(obj, EINA_FALSE);
   ...
   efl_event_callback_add(text_obj, EFL_TEXT_CONTAINER_INTERACTIVE_EVENT_SELECTION_CHANGED,
...
_efl_ui_text_efl_object_destructor(Eo *obj, Efl_Ui_Text_Data *sd)
{
   ...
   efl_event_callback_del(text_obj, EFL_CANVAS_TEXT_EVENT_CHANGED,
         _efl_ui_text_changed_cb, obj);
   efl_event_callback_del(text_obj, EFL_TEXT_CONTAINER_INTERACTIVE_EVENT_SELECTION_CHANGED,
         _efl_ui_text_selection_changed_cb, obj);
...
_efl_ui_text_selection_get(Eo *obj EINA_UNUSED, Efl_Ui_Text_Data *sd)
{
   ...
   efl_text_container_interactive_selection_cursors_get(obj, &start_obj, &end_obj);
...
EOLIAN static void
_efl_ui_text_efl_text_container_interactive_editable_set(Eo *obj, Efl_Ui_Text_Data *sd, Eina_Bool editable)
{
   efl_text_container_interactive_editable_set(efl_super(obj, MY_CLASS), editable);
...
_efl_ui_text_efl_access_object_state_set_get(Eo *obj, Efl_Ui_Text_Data *_pd EINA_UNUSED)
{
   Efl_Access_State_Set ret;
   ret = efl_access_object_state_set_get(efl_super(obj, EFL_UI_TEXT_CLASS));

   if (efl_text_container_interactive_editable_get(obj))
...
_efl_ui_text_efl_access_object_i18n_name_get(Eo *obj, Efl_Ui_Text_Data *pd)
{
   const char *name;
...
_efl_ui_text_editable_efl_object_constructor(Eo *obj, void *_pd EINA_UNUSED)
{
   ...
   if (!elm_widget_theme_klass_get(obj))
     elm_widget_theme_klass_set(obj, "text");
   obj = efl_constructor(efl_super(obj, MY_CLASS));
   efl_text_container_interactive_editable_set(obj, EINA_TRUE);
