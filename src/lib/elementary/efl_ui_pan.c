...
EOLIAN static void
_efl_ui_pan_efl_gfx_object_position_set(Eo *obj, Efl_Ui_Pan_Data *psd, Eina_Position2D pos)
{
...
EOLIAN static void
_efl_ui_pan_efl_gfx_object_size_set(Eo *obj, Efl_Ui_Pan_Data *psd, Eina_Size2D sz)
{
...
EOLIAN static void
_efl_ui_pan_efl_gfx_object_visible_set(Eo *obj, Efl_Ui_Pan_Data *psd, Eina_Bool vis)
{
   if (_evas_object_intercept_call(obj, EVAS_OBJECT_INTERCEPT_CB_VISIBLE, 0, vis))
     return;

   efl_gfx_object_visible_set(efl_super(obj, MY_CLASS), vis);
   if (psd->content) efl_gfx_object_visible_set(psd->content, vis);
}
                           ...
                           void *event_info EINA_UNUSED)
{
   ...
   Eina_Size2D sz = efl_gfx_object_size_get(psd->content);
...
_efl_ui_pan_efl_content_content_set(Evas_Object *obj, Efl_Ui_Pan_Data *psd, Evas_Object *content)
{
   ...
   sz = efl_gfx_object_size_get(psd->content);
   psd->content_w = sz.w;
   psd->content_h = sz.h;
...
{
   efl_gfx_object_position_set(psd->content, EINA_POSITION2D(psd->x - psd->px, psd->y - psd->py));
}
