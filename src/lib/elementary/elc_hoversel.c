...
EOLIAN static void
_elm_hoversel_efl_gfx_object_visible_set(Eo *obj, Elm_Hoversel_Data *sd, Eina_Bool vis)
{
   if (_evas_object_intercept_call(obj, EVAS_OBJECT_INTERCEPT_CB_VISIBLE, 0, vis))
     return;

   efl_gfx_object_visible_set(efl_super(obj, MY_CLASS), vis);
   efl_gfx_object_visible_set(sd->hover, vis);
}
...
EFL_CALLBACKS_ARRAY_DEFINE(_on_parent,
                          ...
                          { EFL_GFX_OBJECT_EVENT_RESIZE, _on_parent_resize });
...
EOLIAN Eina_List*
_elm_hoversel_efl_access_object_access_children_get(Eo *obj EINA_UNUSED, Elm_Hoversel_Data *pd)
{
