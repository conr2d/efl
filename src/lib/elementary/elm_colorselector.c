...
_elm_colorselector_efl_ui_widget_theme_apply(Eo *obj, Elm_Colorselector_Data *sd)
{
   ...
   if ((sd->mode == ELM_COLORSELECTOR_PALETTE) ||
       ...
       (sd->mode == ELM_COLORSELECTOR_BOTH))
     {
        ...
        scale = efl_gfx_object_scale_get(obj) * elm_config_scale_get() / edje_object_base_scale_get(wd->resize_obj);
...
_palette_box_prepare(Eo *o)
{
   ...
   efl_gfx_object_visible_set(o, EINA_FALSE);
}
   ...
   if (sd->focused_item && (sd->focused == ELM_COLORSELECTOR_PALETTE))
     {
       ...
       return efl_gfx_object_geometry_get(VIEW(focus_it));
     }
   else if(sd->focused == ELM_COLORSELECTOR_COMPONENTS)
     return efl_gfx_object_geometry_get(sd->cb_data[sd->sel_color_type]->colorbar);
...
_elm_colorselector_efl_access_object_access_children_get(Eo *obj EINA_UNUSED, Elm_Colorselector_Data *sd)
{
   Eina_List *ret = NULL;
...
_elm_color_item_efl_access_object_state_set_get(Eo *obj, Elm_Color_Item_Data *sd EINA_UNUSED)
{
...
EOLIAN static const char*
_elm_color_item_efl_access_object_i18n_name_get(Eo *eo_it, Elm_Color_Item_Data *it)
{
   Eina_Strbuf *buf;
   const char *color_name = NULL;
   const char *name;
...
{
   return efl_gfx_object_geometry_get(pd->color_obj);
}
