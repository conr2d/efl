...
_anchor_calc(Eo *obj)
{
   ...
   Eina_Rect a_geom = efl_gfx_object_geometry_get(pd->anchor);
   Eina_Rect o_geom = efl_gfx_object_geometry_get(obj);
   Eina_Rect p_geom = efl_gfx_object_geometry_get(ppd->win_parent);
   ...
   if (pd->used_align != EFL_UI_POPUP_ALIGN_NONE)
     efl_gfx_object_position_set(efl_super(obj, EFL_UI_POPUP_CLASS), pos);
}
...
_anchor_del_cb(void *data, const Efl_Event *ev EINA_UNUSED)
{
   ...
   efl_event_callback_del(ppd->win_parent, EFL_GFX_OBJECT_EVENT_RESIZE, _anchor_geom_cb, data);
...
_anchor_detach(Eo *obj)
{
   ...
   efl_event_callback_del(ppd->win_parent, EFL_GFX_OBJECT_EVENT_RESIZE, _anchor_geom_cb, obj);
   efl_event_callback_del(pd->anchor, EFL_GFX_OBJECT_EVENT_RESIZE, _anchor_geom_cb, obj);
   efl_event_callback_del(pd->anchor, EFL_GFX_OBJECT_EVENT_MOVE, _anchor_geom_cb, obj);
...
_efl_ui_popup_anchor_anchor_set(Eo *obj, Efl_Ui_Popup_Anchor_Data *pd, Eo *anchor)
{
   ...
   if (anchor)
     {
        efl_event_callback_add(ppd->win_parent, EFL_GFX_OBJECT_EVENT_RESIZE, _anchor_geom_cb, obj);
        efl_event_callback_add(anchor, EFL_GFX_OBJECT_EVENT_RESIZE, _anchor_geom_cb, obj);
        efl_event_callback_add(anchor, EFL_GFX_OBJECT_EVENT_MOVE, _anchor_geom_cb, obj);
...
EOLIAN static void
_efl_ui_popup_anchor_efl_gfx_object_position_set(Eo *obj, Efl_Ui_Popup_Anchor_Data *pd, Eina_Position2D pos)
{
   _anchor_detach(obj);
