...
_segment_draw(Efl_Ui_Textpath_Data *pd, int slice_no, int w1, int w2, int cmp, Evas_Map *map, Eina_Bezier bezier)
{
   ...
   len = w2 - w1;
   r = efl_gfx_object_geometry_get(pd->text_obj);
...
_text_draw(Efl_Ui_Textpath_Data *pd)
{
   ...
   sz = efl_gfx_object_size_get(pd->text_obj);
   if (pd->autofit)
     remained_w = sz.w;
   else
...
_path_data_get(Eo *obj, Efl_Ui_Textpath_Data *pd, Eina_Bool set_min)
{
   ...
   opos = efl_gfx_object_position_get(obj);
...
_ellipsis_set(Efl_Ui_Textpath_Data *pd)
{
   ...
   efl_gfx_object_size_set(pd->text_obj, EINA_SIZE2D(w,  h));
...
_efl_ui_textpath_efl_canvas_group_group_add(Eo *obj, Efl_Ui_Textpath_Data *priv)
{
   ...
   efl_gfx_object_visible_set(priv->text_obj, EINA_TRUE);
...
EOLIAN static void
_efl_ui_textpath_efl_gfx_object_position_set(Eo *obj, Efl_Ui_Textpath_Data *pd, Eina_Position2D pos)
{
...
EOLIAN static void
_efl_ui_textpath_efl_gfx_object_size_set(Eo *obj, Efl_Ui_Textpath_Data *pd EINA_UNUSED, Eina_Size2D sz)
{
