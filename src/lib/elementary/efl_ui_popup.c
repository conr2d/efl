...
EOLIAN static void
_efl_ui_popup_efl_gfx_object_position_set(Eo *obj, Efl_Ui_Popup_Data *pd EINA_UNUSED, Eina_Position2D pos)
{
...
_calc_align(Eo *obj)
{
   ...
   Eina_Rect p_geom = efl_gfx_object_geometry_get(pd->win_parent);

   Eina_Rect o_geom = efl_gfx_object_geometry_get(obj);
   ...
   switch (pd->align)
     {
      case EFL_UI_POPUP_ALIGN_CENTER:
         efl_gfx_object_position_set(efl_super(obj, MY_CLASS), EINA_POSITION2D((pw - ow ) / 2, (ph - oh) / 2));
         break;
      case EFL_UI_POPUP_ALIGN_LEFT:
         efl_gfx_object_position_set(efl_super(obj, MY_CLASS), EINA_POSITION2D(0, (ph - oh) / 2));
         break;
      case EFL_UI_POPUP_ALIGN_RIGHT:
         efl_gfx_object_position_set(efl_super(obj, MY_CLASS), EINA_POSITION2D(pw - ow, (ph - oh) / 2));
         break;
      case EFL_UI_POPUP_ALIGN_TOP:
         efl_gfx_object_position_set(efl_super(obj, MY_CLASS), EINA_POSITION2D((pw - ow) / 2, 0));
         break;
      case EFL_UI_POPUP_ALIGN_BOTTOM:
         efl_gfx_object_position_set(efl_super(obj, MY_CLASS), EINA_POSITION2D((pw - ow) / 2, ph - oh));
         break;
      default:
         break;
     }
...
EOLIAN static void
_efl_ui_popup_efl_gfx_object_size_set(Eo *obj, Efl_Ui_Popup_Data *pd EINA_UNUSED, Eina_Size2D size)
{
   efl_gfx_object_size_set(efl_super(obj, MY_CLASS), size);
...
_efl_ui_popup_efl_ui_widget_widget_parent_set(Eo *obj, Efl_Ui_Popup_Data *pd EINA_UNUSED, Eo *parent EINA_UNUSED)
{
   ...
   efl_gfx_object_position_set(pd->backwall, EINA_POSITION2D(p_geom.x, p_geom.y));
   efl_gfx_object_size_set(pd->backwall, EINA_SIZE2D(p_geom.w, p_geom.h));

   efl_event_callback_add(pd->win_parent, EFL_GFX_OBJECT_EVENT_RESIZE, _parent_geom_cb, obj);
   efl_event_callback_add(pd->win_parent, EFL_GFX_OBJECT_EVENT_MOVE, _parent_geom_cb, obj);
}
...
EOLIAN static void
_efl_ui_popup_efl_gfx_object_visible_set(Eo *obj, Efl_Ui_Popup_Data *pd, Eina_Bool v)
{
   if (_evas_object_intercept_call(obj, EVAS_OBJECT_INTERCEPT_CB_VISIBLE, 0, v))
     return;

   efl_gfx_object_visible_set(efl_super(obj, MY_CLASS), v);
...
_efl_ui_popup_efl_object_destructor(Eo *obj, Efl_Ui_Popup_Data *pd)
{
   ...
   efl_event_callback_del(pd->win_parent, EFL_GFX_OBJECT_EVENT_RESIZE, _parent_geom_cb,
                          obj);
   efl_event_callback_del(pd->win_parent, EFL_GFX_OBJECT_EVENT_MOVE, _parent_geom_cb,
                          obj);
   ...
   if (pd->needs_group_calc)
     {
        ...
        efl_gfx_object_position_set(pd->backwall, EINA_POSITION2D(0, 0));
        efl_gfx_object_size_set(pd->backwall, EINA_SIZE2D(p_geom.w, p_geom.h));
