...
EOLIAN static void
_evas_text_efl_gfx_object_size_set(Eo *eo_obj, Evas_Text_Data *o, Eina_Size2D sz)
{
   Evas_Object_Protected_Data *obj = efl_data_scope_get(eo_obj, EFL_CANVAS_OBJECT_CLASS);
...
_evas_text_efl_object_dbg_info_get(Eo *eo_obj, Evas_Text_Data *o EINA_UNUSED, Efl_Dbg_Info *root)
{
   ...
   text = efl_text_container_text_get(eo_obj);
   EFL_DBG_INFO_APPEND(group, "Text", EINA_VALUE_TYPE_STRING, text);
}

EOLIAN static void
_evas_text_efl_text_container_text_set(Eo *eo_obj, Evas_Text_Data *o, const char *_text)
{
   Evas_Object_Protected_Data *obj = efl_data_scope_get(eo_obj, EFL_CANVAS_OBJECT_CLASS);
   int len;
   Eina_Unicode *text;
...
EOLIAN static const char*
_evas_text_efl_text_container_text_get(Eo *eo_obj EINA_UNUSED, Evas_Text_Data *o)
{
...
_evas_text_style_set(Eo *eo_obj, Evas_Text_Data *o, Evas_Text_Style_Type style)
{
   ...
   efl_gfx_object_size_set(efl_super(eo_obj, MY_CLASS), EINA_SIZE2D(w,  h));
   evas_object_change(eo_obj, obj);
}
...
EOLIAN static void
_evas_text_efl_gfx_object_scale_set(Evas_Object *eo_obj, Evas_Text_Data *o,
                                 double scale)
{
   int size;
   const char *font;
        ...
        if (o->cur.ellipsis >= 0.0)
          {
             ...
             efl_gfx_object_size_set(efl_super(eo_obj, MY_CLASS), EINA_SIZE2D(min,  h + t + b));
          }
        else
          {
             efl_gfx_object_size_set(efl_super(eo_obj, MY_CLASS), EINA_SIZE2D(w + l + r,  h + t + b));
          }
////        obj->cur->cache.geometry.validity = 0;
   ...
   else
     {
        int t = 0, b = 0, l = 0, r = 0;
        _evas_object_text_pad_get(eo_obj, o, &l, &r, &t, &b);
        efl_gfx_object_size_set(efl_super(eo_obj, MY_CLASS), EINA_SIZE2D(0,  o->max_ascent + o->max_descent + t + b));
////        obj->cur->cache.geometry.validity = 0;
     }
   o->last_computed.w = obj->cur->geometry.w;
...
{
   efl_text_container_text_set((Eo *) obj, text);
}

EAPI const char *
evas_object_text_text_get(const Eo *obj)
{
   return efl_text_container_text_get((Eo *) obj);
}
