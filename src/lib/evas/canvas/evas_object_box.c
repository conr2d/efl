...
EFL_CALLBACKS_ARRAY_DEFINE(evas_object_box_callbacks,
  { EFL_GFX_OBJECT_EVENT_RESIZE, _on_child_resize },
  { EFL_EVENT_DEL, _on_child_del },
  { EFL_GFX_OBJECT_EVENT_CHANGE_SIZE_HINTS, _on_child_hints_changed }
);
...
EOLIAN static void
_evas_box_efl_gfx_object_size_set(Eo *o, Evas_Object_Box_Data *_pd EINA_UNUSED, Eina_Size2D sz)
{
   if (_evas_object_intercept_call(o, EVAS_OBJECT_INTERCEPT_CB_RESIZE, 0, sz.w, sz.h))
     return;

   efl_gfx_object_size_set(efl_super(o, MY_CLASS), sz);
   evas_object_smart_changed(o);
}

EOLIAN static void
_evas_box_efl_gfx_object_position_set(Eo *o, Evas_Object_Box_Data *_pd EINA_UNUSED, Eina_Position2D pos)
{
   if (_evas_object_intercept_call(o, EVAS_OBJECT_INTERCEPT_CB_MOVE , 0, pos.x, pos.y))
     return;

   efl_gfx_object_position_set(efl_super(o, MY_CLASS), pos);
   evas_object_smart_changed(o);
}
