...
_update_vgtree_viewport(Eo *obj, Efl_Canvas_Vg_Object_Data *pd)
{
   ...
   Eina_Size2D sz = efl_gfx_object_size_get(obj);
   Eina_Matrix3 m;
...
_efl_canvas_vg_object_root_node_get(Eo *obj EINA_UNUSED, Efl_Canvas_Vg_Object_Data *pd)
{
   Efl_VG *root = NULL;
...
_efl_canvas_vg_object_fill_mode_get(Eo *obj EINA_UNUSED, Efl_Canvas_Vg_Object_Data *pd)
{
   return pd->fill_mode;
}
        ...
        if (!eina_rectangle_is_empty(&pd->viewbox.rect))
          {
             ...
             efl_event_callback_del(obj, EFL_GFX_OBJECT_EVENT_RESIZE, _evas_vg_resize, pd);
          }
        return;
     }
   // register for resize callback if not done yet
   if (eina_rectangle_is_empty(&pd->viewbox.rect))
     efl_event_callback_add(obj, EFL_GFX_OBJECT_EVENT_RESIZE, _evas_vg_resize, pd);
...
_efl_canvas_vg_object_viewbox_get(Eo *obj EINA_UNUSED, Efl_Canvas_Vg_Object_Data *pd)
{
   return pd->viewbox;
}
...
EOLIAN static void
_efl_canvas_vg_object_viewbox_align_get(Eo *obj EINA_UNUSED, Efl_Canvas_Vg_Object_Data *pd, double *align_x, double *align_y)
{
...
EOLIAN static void
_efl_canvas_vg_object_efl_file_file_get(Eo *obj EINA_UNUSED, Efl_Canvas_Vg_Object_Data *pd, const char **file, const char **key)
{
   if (pd->vg_entry)
     {
