        ...
        if (clipper != eo_obj)
          {
             ...
             if (!had_clippees && smart->cur->visible)
               efl_gfx_object_visible_set(clipper, 1);
          }
     }
   ...
   if (o->clipped)
     {
        ...
        if (!efl_canvas_object_clipees_has(clipper))
          efl_gfx_object_visible_set(clipper, 0);
     }
...
EOLIAN static void
_efl_canvas_group_efl_gfx_object_visible_set(Eo *eo_obj, Evas_Smart_Data *o, Eina_Bool vis)
{
   if (_evas_object_intercept_call(eo_obj, EVAS_OBJECT_INTERCEPT_CB_VISIBLE, 0, vis))
     return;

   efl_gfx_object_visible_set(efl_super(eo_obj, MY_CLASS), vis);
   ...
   if (o->clipped)
     {
        ...
        efl_gfx_object_visible_set(clipper, vis);
     }
}

EOLIAN static void
_efl_canvas_group_efl_gfx_object_position_set(Eo *eo_obj, Evas_Smart_Data *o, Eina_Position2D pos)
{
