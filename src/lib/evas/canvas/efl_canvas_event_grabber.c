...
_efl_canvas_event_grabber_efl_canvas_group_group_member_add(Eo *eo_obj, Efl_Object_Event_Grabber_Data *pd, Eo *eo_sub)
{
   ...
   if (eo_sub != pd->rect)
     efl_event_callback_add(eo_sub, EFL_GFX_OBJECT_EVENT_RESTACK, _efl_canvas_object_event_grabber_child_restack, pd);
}
...
_efl_canvas_event_grabber_efl_canvas_group_group_member_del(Eo *eo_obj EINA_UNUSED, Efl_Object_Event_Grabber_Data *pd, Eo *member)
{
   ...
   efl_event_callback_del(member, EFL_GFX_OBJECT_EVENT_RESTACK, _efl_canvas_object_event_grabber_child_restack, pd);
   pd->contained = eina_list_remove(pd->contained, obj);
...
EOLIAN static Eina_Bool
_efl_canvas_event_grabber_efl_canvas_group_group_need_recalculate_get(Eo *eo_obj EINA_UNUSED, Efl_Object_Event_Grabber_Data *pd EINA_UNUSED)
{
   return EINA_FALSE;
}

EOLIAN static void
_efl_canvas_event_grabber_efl_gfx_object_position_set(Eo *eo_obj, Efl_Object_Event_Grabber_Data *pd, Eina_Position2D pos)
{
   efl_gfx_object_position_set(efl_super(eo_obj, MY_CLASS), pos);
   efl_gfx_object_position_set(pd->rect, pos);
}

EOLIAN static void
_efl_canvas_event_grabber_efl_gfx_object_size_set(Eo *eo_obj, Efl_Object_Event_Grabber_Data *pd, Eina_Size2D sz)
{
   efl_gfx_object_size_set(efl_super(eo_obj, MY_CLASS), sz);
   efl_gfx_object_size_set(pd->rect, sz);
}

EOLIAN static Eina_Bool
_efl_canvas_event_grabber_efl_gfx_object_visible_get(Eo *eo_obj EINA_UNUSED, Efl_Object_Event_Grabber_Data *pd)
{
   return pd->vis;
}

EOLIAN static void
_efl_canvas_event_grabber_efl_gfx_object_visible_set(Eo *eo_obj EINA_UNUSED, Efl_Object_Event_Grabber_Data *pd, Eina_Bool set)
{
   if (set)
     {
        Evas_Object_Protected_Data *obj;
   ...
   pd->vis = !!set;
   efl_gfx_object_visible_set(pd->rect, set);
...
_efl_canvas_event_grabber_efl_object_constructor(Eo *eo_obj, Efl_Object_Event_Grabber_Data *pd)
{
   ...
   efl_event_callback_add(eo_obj, EFL_GFX_OBJECT_EVENT_RESTACK, _efl_canvas_object_event_grabber_restack, pd);
...
EOLIAN static Eina_Bool
_efl_canvas_event_grabber_freeze_when_visible_get(Eo *eo_obj EINA_UNUSED, Efl_Object_Event_Grabber_Data *pd)
{
   return pd->freeze;
}
